<?php
class CommunesLivraisonController extends Controller {
    
    public function liste(){
      conf::redir();
      $this->loadmodel('FraisLivraison');
      $_SESSION['bo_menu'] = 'FraisLivraison';  $_SESSION['bo_sub_menu'] = 'CommunesLivraison';
      $d['numero_page_encours']=1;
      //Total produits
      $d['communes']['total'] = $this->FraisLivraison->findCountAll('livraison_destinations');

      ///total  non actives
      $d['communes']['non_actifs'] = $this->FraisLivraison->findCount( array( 'statut' => 0 ),'livraison_destinations' );

      //total categories actives
      $d['communes']['actifs'] = $this->FraisLivraison->findCount( array( 'statut' => 1 ),'livraison_destinations' ); 

      //communes liées à aucune livraison
      $d['communes']['no_order'] = $this->getNeverOrderPlace()['nbre'];

      //liste de nombre de commande par id destination
      $d['communes']['liste_order'] = $this->getNeverOrderPlace()['liste'];

      $d['nombre_pages']=ceil( $d['communes']['total'] / 10 );
      
      //Liste des categories produits
      $d['communes']['liste'] = $this->FraisLivraison->find(array(
    ...
    private function getNeverOrderPlace(){
      $req = [
          ...
          ,'condition' => "commandes.id_livraison_destination is null"
          ];    
      $nbre = count( $this->FraisLivraison->findLeftJoin( $req,'livraison_destinations','commandes') );
      unset($req['condition']);

      $listeSql = $this->FraisLivraison->findLeftJoin( $req,'livraison_destinations','commandes');
      // debug($listeSql);
      $listeArray = array();
    ...
    public function modifier( $token ){
      ...
      if( !isset($token) || empty($token) ){
        header('Location: '.BASE_URL.DS.'communesLivraison/liste');
      }else{
        //recupere les informations de la categorie
        $d['commune'] = current( $this->FraisLivraison->find(array(
              'condition' => 'token = "'.$token.'"', //
              'limit' => '0,1'
            ),'livraison_destinations') );

        if( empty($d['commune']) ){ //verifie que la categorie existe
          header('Location: '.BASE_URL.DS.'communesLivraison/liste'); // renvoi vers liste de categorie si la categorie n'existe pas
        }
    ...
    public function ajouter(){
      conf::redir();
      $this->loadmodel('FraisLivraison');
      $_SESSION['bo_menu'] = 'FraisLivraison';  $_SESSION['bo_sub_menu'] = 'Categories';
      $d = array();
    ...
    public function details($token){
      ...
      if( !isset($token) || empty($token) ){
        ...
        $d['categorie'] = current( $this->FraisLivraison->find(array(
              'order' => array('champs' => 'nom','param' => 'ASC'),
              'condition' => 'token = "'.$token.'"', //
              'limit' => '0,1'
            ),'categories_produits') );

        $d['produits'] = $this->FraisLivraison->findJoin(array(
    ...
    private function unit_mesure($condition=null){
      //recuperer les unité de mésure
          $unites_from_bd = $this->FraisLivraison->find(array(
    ...
    private function get_category($condition=null){
      //recuperer les unité de mésure
          ...
          // debug($requette);          
          // debug( $this->FraisLivraison->find( $requette ,'categories_produits' ) );          
          $category_from_bd = $this->FraisLivraison->find( $requette ,'categories_produits' );
    ...
    private function get_categories($filter){
      $categories = $this->FraisLivraison->find(array(
