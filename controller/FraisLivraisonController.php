...
class FraisLivraisonController extends Controller {
    ...
    public function liste(){
      ...
      $_SESSION['bo_menu'] = 'FraisLivraison';  $_SESSION['bo_sub_menu'] = 'FraisLivraison';
      $d['numero_page_encours']=1;
      //Total produits
      $total = $this->FraisLivraison->findCountAll('frais_livraison');     

      $d['fraisLivraison']['total'] = $total;
      $d['nombre_pages']=ceil( $total / 10 );
      
      //Liste des categories produits
      $d['fraisLivraison']['liste'] = $this->FraisLivraison->find(array(
            'order' => array('champs' => 'min','param' => 'ASC')
            ),'frais_livraison');
    ...
    public function modifier( $token ){
      ...
      if( !isset($token) || empty($token) ){
        header('Location: '.BASE_URL.DS.'fraisLivraison/liste');
      }else{
        //recupere les informations de la categorie
        $d['fee'] = current( $this->FraisLivraison->find(array(
              'order' => array('champs' => 'id','param' => 'DESC'),
              'condition' => 'token = "'.$token.'"', //
              'limit' => '0,1'
            ),'frais_livraison') );

        if( empty($d['fee']) ){ //verifie que la categorie existe
          header('Location: '.BASE_URL.DS.'fraisLivraison/liste'); // renvoi vers liste de categorie si la categorie n'existe pas
        }
    ...
    public function ajouter(){
      conf::redir();
      $this->loadmodel('FraisLivraison');
      $_SESSION['bo_menu'] = 'FraisLivraison';  $_SESSION['bo_sub_menu'] = 'unitesMesure';
      $d = array();
